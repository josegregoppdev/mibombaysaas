...
class RegistroEmpresaServiceTest {
    ...
    @Test
    void registrar_generaTenantIdConFormatoCorrecto() {
        EmpresaDTORequest dto = crearEmpresaDTORequest();
        Empresa empresa = crearEmpresa();

        when(empresaRepository.existsBySubdominio(any())).thenReturn(false);
    ...
    @Test
    void registrar_generaEmailCajeroCorrectamente() {
        EmpresaDTORequest dto = crearEmpresaDTORequest();
        Empresa empresa = crearEmpresa();
        ...
        Usuario cajero = captor.getAllValues().get(1);

        assertNotNull(cajero);
        assertEquals("admin_cajero@test.com", cajero.getEmail());
        assertEquals(Rol.CAJERO, cajero.getRol());
        assertTrue(cajero.getDebeCambiarPassword());
    }

    @Test
    void registrar_hasheaDocumentoEncargado() {
        EmpresaDTORequest dto = crearEmpresaDTORequest();
        Empresa empresa = crearEmpresa();
        ...
        verify(passwordEncoder).encode(dto.getDocumentoEncargado());
    }
}
