package com.josegregoppdev.mibombay.controller.recipe;

import com.josegregoppdev.mibombay.common.tenant.TenantContext;
import com.josegregoppdev.mibombay.dto.recipe.RecipeDTO;
import com.josegregoppdev.mibombay.service.ingredient.IngredientService;
import com.josegregoppdev.mibombay.service.recipe.RecipeService;
import org.junit.jupiter.api.AfterEach;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.InjectMocks;
import org.mockito.Mock;
import org.mockito.junit.jupiter.MockitoExtension;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageImpl;
import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.ui.ExtendedModelMap;
import org.springframework.ui.Model;
import org.springframework.validation.BindingResult;
import org.springframework.web.servlet.mvc.support.RedirectAttributes;

import java.util.List;

import static com.josegregoppdev.mibombay.testdata.TestDataFactory.createRecipeDTO;
import static org.junit.jupiter.api.Assertions.*;
import static org.mockito.ArgumentMatchers.*;
import static org.mockito.Mockito.*;

@ExtendWith(MockitoExtension.class)
class RecipeControllerTest {

    @Mock
    private RecipeService recipeService;

    @Mock
    private IngredientService ingredientService;

    @InjectMocks
    private RecipeController controller;

    private static final String TENANT_ID = "tnt_test1234567890123456789012345678";

    @BeforeEach
    void setUp() {
        TenantContext.set(TENANT_ID);
    }

    @AfterEach
    void tearDown() {
        TenantContext.clear();
    }

    @Test
    void list_noFilters_addsAttributesAndReturnsView() {
        Pageable pageable = PageRequest.of(0, 20);
        RecipeDTO dto = createRecipeDTO();
        Page<RecipeDTO> page = new PageImpl<>(List.of(dto));
        when(recipeService.getPaginatedRecipes(eq(TENANT_ID), isNull(), any(Pageable.class)))
                .thenReturn(page);

        Model model = new ExtendedModelMap();
        String view = controller.list(null, pageable, model);

        assertEquals("recipe/list", view);
        assertTrue(model.containsAttribute("page"));
        assertTrue(model.containsAttribute("name"));
    }

    @Test
    void list_withNameFilter_passesParametersToService() {
        Pageable pageable = PageRequest.of(0, 20);
        when(recipeService.getPaginatedRecipes(eq(TENANT_ID), eq("Classic"), any(Pageable.class)))
                .thenReturn(Page.empty());

        Model model = new ExtendedModelMap();
        String view = controller.list("Classic", pageable, model);

        assertEquals("recipe/list", view);
        assertEquals("Classic", model.getAttribute("name"));
    }

    @Test
    void showForm_addsAttributesAndReturnsView() {
        Model model = new ExtendedModelMap();
        String view = controller.showForm(model);

        assertEquals("recipe/form", view);
        assertTrue(model.containsAttribute("recipe"));
        assertTrue(model.containsAttribute("ingredients"));
    }

    @Test
    void create_success_redirectsWithMessage() {
        RecipeDTO dto = createRecipeDTO();
        BindingResult bindingResult = mock(BindingResult.class);
        when(bindingResult.hasErrors()).thenReturn(false);

        String view = controller.create(dto, bindingResult, mock(RedirectAttributes.class), new ExtendedModelMap());

        assertEquals("redirect:/recipe", view);
        verify(recipeService).createNewRecipe(dto, TENANT_ID);
    }

    @Test
    void create_withValidationError_returnsFormView() {
        RecipeDTO dto = createRecipeDTO();
        BindingResult bindingResult = mock(BindingResult.class);
        when(bindingResult.hasErrors()).thenReturn(true);

        Model model = new ExtendedModelMap();
        String view = controller.create(dto, bindingResult, mock(RedirectAttributes.class), model);

        assertEquals("recipe/form", view);
        assertTrue(model.containsAttribute("ingredients"));
    }

    @Test
    void create_withException_redirectsWithError() {
        RecipeDTO dto = createRecipeDTO();
        BindingResult bindingResult = mock(BindingResult.class);
        when(bindingResult.hasErrors()).thenReturn(false);
        doThrow(new IllegalArgumentException("A recipe with that code already exists"))
                .when(recipeService).createNewRecipe(dto, TENANT_ID);

        String view = controller.create(dto, bindingResult, mock(RedirectAttributes.class), new ExtendedModelMap());

        assertEquals("redirect:/recipe/new", view);
    }

    @Test
    void showEditForm_exists_addsAttributesAndReturnsView() {
        RecipeDTO dto = createRecipeDTO();
        when(recipeService.getRecipeById(1L, TENANT_ID)).thenReturn(dto);

        Model model = new ExtendedModelMap();
        String view = controller.showEditForm(1L, model);

        assertEquals("recipe/form", view);
        assertTrue(model.containsAttribute("recipe"));
        assertTrue(model.containsAttribute("ingredients"));
    }

    @Test
    void showEditForm_doesNotExist_redirectsToList() {
        when(recipeService.getRecipeById(99L, TENANT_ID))
                .thenThrow(new IllegalArgumentException("Recipe not found"));

        Model model = new ExtendedModelMap();
        String view = controller.showEditForm(99L, model);

        assertEquals("redirect:/recipe", view);
    }

    @Test
    void update_success_redirectsWithMessage() {
        RecipeDTO dto = createRecipeDTO();
        BindingResult bindingResult = mock(BindingResult.class);
        when(bindingResult.hasErrors()).thenReturn(false);

        String view = controller.update(1L, dto, bindingResult, mock(RedirectAttributes.class), new ExtendedModelMap());

        assertEquals("redirect:/recipe", view);
        verify(recipeService).updateExistingRecipe(1L, dto, TENANT_ID);
    }

    @Test
    void update_withValidationError_returnsFormView() {
        RecipeDTO dto = createRecipeDTO();
        BindingResult bindingResult = mock(BindingResult.class);
        when(bindingResult.hasErrors()).thenReturn(true);

        Model model = new ExtendedModelMap();
        String view = controller.update(1L, dto, bindingResult, mock(RedirectAttributes.class), model);

        assertEquals("recipe/form", view);
        assertTrue(model.containsAttribute("ingredients"));
    }

    @Test
    void update_withException_redirectsWithError() {
        RecipeDTO dto = createRecipeDTO();
        BindingResult bindingResult = mock(BindingResult.class);
        when(bindingResult.hasErrors()).thenReturn(false);
        doThrow(new IllegalArgumentException("A recipe with that code already exists"))
                .when(recipeService).updateExistingRecipe(1L, dto, TENANT_ID);

        String view = controller.update(1L, dto, bindingResult, mock(RedirectAttributes.class), new ExtendedModelMap());

        assertEquals("redirect:/recipe/1/edit", view);
    }

    @Test
    void toggleStatus_success_redirectsWithMessage() {
        String view = controller.toggleStatus(1L, mock(RedirectAttributes.class));

        assertEquals("redirect:/recipe", view);
        verify(recipeService).toggleRecipeActiveStatus(1L, TENANT_ID);
    }

    @Test
    void toggleStatus_doesNotExist_redirectsWithError() {
        doThrow(new IllegalArgumentException("Recipe not found"))
                .when(recipeService).toggleRecipeActiveStatus(99L, TENANT_ID);

        String view = controller.toggleStatus(99L, mock(RedirectAttributes.class));

        assertEquals("redirect:/recipe", view);
    }
}
